...
import { TimelineLite,Draggable,TweenMax} from "gsap/all";
...
class SimpleTween extends Component {
	...
	componentDidMount(){
		...
		this.logoTween = new TimelineLite({ paused:true })
			.to(this.logoContainer, 2, { x: 500, onUpdate:this.checkHit})
        this.logoTween2 = new TimelineLite({ paused:true })
			.from(this.logoContainer2, 2, { x: 500})
	}
    checkHit()  {
        if (Draggable.hitTest(this.logoContainer,this.logoContainer2)){
          TweenMax.killTweensOf(this.logoContainer,this.logoContainer2);
